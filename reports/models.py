...
class Report(models.Model):
    ...
    Attributes:
        owner (ForeignKey): The user who created the report.
                                Related to :model:`auth.User`.
        post (ForeignKey): The post that is being reported.
                             Related to :model:`posts.Post`.
        reason (CharField): The reason for the report, chosen
                                      from predefined options.
        custom_reason (TextField): An optional custom reason provided by the
                            user if the predefined options are not sufficient.
        created_at (DateTimeField): The timestamp when the report was created.
        updated_at (DateTimeField): The timestamp when the report
                                                 was last updated.
    Meta:
        ordering (list): Orders reports by the `created_at`
